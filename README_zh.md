...
| 芯片 | Apple Silicon (M1/M2/M3/M4) | Intel/AMD x86_64（需 NVIDIA GPU） |
| 磁盘空间 | 约 6 GB（应用 + 模型） | 约 11 GB（应用 + 模型 + CUDA runtime；runtime 下载约 2.5 GB） |
| 推理后端 | 默认 MPS（Apple Silicon） | CUDA（需 NVIDIA GPU） |
