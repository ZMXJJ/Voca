...
- **Model catalog wired to GGUF (done):** added a `voxcpm2_gguf` catalog entry → `DennisHuang648/VoxCPM2-GGUF` with `allowPatterns` (`*BaseLM*Q8_0*.gguf`, `*Acoustic*.gguf`) so only the Q8 BaseLM + Acoustic (~3.3 GB) are fetched. Added `allowPatterns`/`ignorePatterns` to `ProviderInfo` (Python + TS contract) threaded into HF + ModelScope `snapshot_download`; a GGUF arm in `bootstrap_assets._is_voxcpm_ready` (requires a BaseLM **and** an Acoustic `.gguf`, else falls through to the safetensors checks); and transparent key routing `cpp_tts_backend.resolve_model_key` (`voxcpm2`→`voxcpm2_gguf`) applied in `generate_audio` when `VOCA_TTS_BACKEND=cpp`. Verified end-to-end: `modelKey=voxcpm2` under the cpp backend generates 48 kHz audio from the GGUF dir on Metal. i18n added for the new entry (en/zh-CN/zh-TW).

### Still open
- **Multi-backend builds:** Metal verified. CUDA (`-DGGML_CUDA=ON`) and Vulkan (`-DGGML_VULKAN=ON`) builds + per-platform bundling of the native binary and ggml libs still to do. The same PAD fix applies to all backends (it removed a non-portable op); the residency fix is Metal-only. Server test runbooks shipped: `llama.cpp-omni/tools/omni/voxcpm2/TESTING-CUDA.md` and `TESTING-VULKAN.md`.
- **Bootstrap default is still safetensors under cpp.** The `voxcpm2_gguf` entry is **on-demand** (download the "VoxCPM2 (GGUF)" model from Model Management; generation gives a clear error if it's missing). `BOOTSTRAP_MODEL_KEYS` and the **Rust** readiness mirror (`bootstrap.rs`) still target the safetensors `voxcpm2`, so a fresh `VOCA_TTS_BACKEND=cpp` install downloads ~5 GB safetensors at onboarding **and** the GGUF on first use (~8 GB, safetensors unused). Making GGUF the bootstrap default requires a lockstep Python+Rust change to `bootstrap_entries`/`BOOTSTRAP_MODEL_KEYS`/the Rust local readiness check — deliberately deferred (Rust onboarding can't be fully verified without building the Tauri app).
- **No ModelScope mirror for the GGUF repo.** `voxcpm2_gguf` is HF-only; CN users silently fall back to HF (slower, still works). The `allowPatterns` plumbing already supports ModelScope, so mirroring `DennisHuang648/VoxCPM2-GGUF` to a ModelScope repo + adding the provider is all that's needed.
- **Upstreaming:** the C++ fixes + test docs live on the fork `ZMXJJ/llama.cpp-omni` (branch `feat/voxcpm2-continuation-clone`); offer them back to `llama.cpp-omni`.
