...
- **Windows: Vulkan shipped via CI; CUDA deferred (impractical to build).** A GitHub Actions workflow (`.github/workflows/windows-build.yml`) compiles the fork on `windows-2022` and produces the NSIS installer. **Vulkan is built and shipped** — a working `Voca_0.5.0_x64-setup.exe` (185 MB) with the bundled static `llama-tts-server` (Vulkan) + denoiser was produced and the sidecar's device-switch falls back to Vulkan on any GPU (incl. NVIDIA via the driver's `vulkan-1.dll`), so Windows is functional. **CUDA proved impractical in CI:** with `GGML_CUDA_FORCE_MMQ=ON` (chosen to avoid bundling ~500 MB of cuBLAS DLLs), the MMQ kernel compile runs **60–85+ min** on the 2-core runner even with pinned archs + `GGML_CUDA_FA=OFF`, and the alternative (drop MMQ, use cuBLAS) refills the ~500 MB DLL cost. The CUDA path is wired and opt-in (`-f cuda=true`) but needs **build caching (sccache)** to be sustainable — deferred. Given VoxCPM2 is small and Vulkan runs on NVIDIA too, Vulkan-only matches the reference (MiniCPM-Desk-Pet ships Vulkan-only). Legacy Windows torch/`cuda_upgrade` overlay can be retired now that Vulkan ships. **Still to verify:** first-launch + device-switched generation on a real Windows machine (NVIDIA + non-NVIDIA).
